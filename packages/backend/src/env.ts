...
export const env = {
  ...
  bullPrefix: process.env.BULL_PREFIX ?? 'tgcasino',
  ...
  referralCooldownSeconds: Number(process.env.REFERRAL_COOLDOWN_SECONDS ?? '86400'),
  maxReferralsPerDevicePerDay: Number(process.env.MAX_REFERRALS_PER_DEVICE_PER_DAY ?? '3'),

  stakingAprBps: Number(process.env.STAKING_APR_BPS ?? '1200'),
  stakingMinLockSeconds: Number(process.env.STAKING_MIN_LOCK_SECONDS ?? '86400'),
};
