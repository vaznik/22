...
bot.start(async (ctx) => {
  await ctx.reply(
    ...
    {
      reply_markup: {
        inline_keyboard: [[{ text: 'Open Casino', web_app: { url: env.publicWebappUrl } }]],
      },
    },
  );
});
...
bot.on('successful_payment', async (ctx) => {
  ...
  await ctx.reply('✅ Deposit credited! Open the Mini App to play.', {
    reply_markup: { inline_keyboard: [[{ text: 'Open Casino', web_app: { url: env.publicWebappUrl } }]] },
  });
});
...
server.listen(port, async () => {
  ...
  // Set webhook to Render URL: you will do it once via curl (see docs/deploy.md)
});
